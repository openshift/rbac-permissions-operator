...
Quick reference for developing the Rbac Permissions.
...
```bash
# Clone repository
git clone https://github.com/openshift/rbac-permissions-operator.git
cd rbac-permissions-operator
...
go test ./controllers/subjectpermission/...  # Test specific package
...
```bash
# Run specific test
ginkgo -focus="NetworkPolicy" ./controllers/subjectpermission/

# Run tests for one package
go test -v ./controllers/subjectpermission/
...
- **Controllers**: `controllers/{subjectpermission,namespace}/` - Reconciliation logic
- **Business Logic**: `controllers/subjectpermission/` - Resource management

...
Testing guidelines for the Rbac Permissions.
...
go test ./controllers/subjectpermission/
...
**Minimal validation:**
```bash
# After changing controllers/subjectpermission/
go test ./controllers/subjectpermission/
```
...
The `go-test` hook is **not enabled** in the current pre-commit config because it's too slow for pre-commit:
```yaml
# Not enabled (too slow):
# - id: go-test
#   entry: make go-test
#   files: '\.go$'
```

Run `make go-test` manually before pushing.

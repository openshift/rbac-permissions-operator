...
# clone bundle repo
SAAS_OPERATOR_DIR="saas-rbac-permissions-operator-bundle"
BUNDLE_DIR="$SAAS_OPERATOR_DIR/rbac-permissions-operator/"

rm -rf "$SAAS_OPERATOR_DIR"

git clone \
    --branch "$BRANCH_CHANNEL" \
    https://app:"${APP_SRE_BOT_PUSH_TOKEN}"@gitlab.cee.redhat.com/service/saas-rbac-permissions-operator-bundle.git \
    "$SAAS_OPERATOR_DIR"
...
# create package yaml
cat <<EOF > $BUNDLE_DIR/rbac-permissions-operator.package.yaml
packageName: rbac-permissions-operator
channels:
- name: $BRANCH_CHANNEL
  currentCSV: rbac-permissions-operator.v${NEW_VERSION}
EOF
...
REGISTRY_IMG="quay.io/app-sre/rbac-permissions-operator-registry"

...
func TestValidSafeList(t *testing.T) {
	// mock namespacelist
	...
	for i, v := range expectedSafeList {
		if v != safeList[i] {
			t.Errorf("got %s, want %s", safeList, expectedSafeList)
		}
	}
...
func TestCreateValidRoleBinding(t *testing.T) {
	...
	newRoleBinding := controllerutil.NewRoleBindingForClusterRole("examplePermissionClusterRoleName", "exampleGroupName", "Group", "examplenamespace")
...
// 	buildCondition := controllerutil.UpdateCondition(mockSubjectPermission(), "testMessage", []string{"testClusterRoleName"}, false, "testState")
...
// 	if con0.ClusterRoleNames != con1.ClusterRoleNames {

...
func DeletePrometheusMetric(gp *managedv1alpha1.GroupPermission) {
...
// permission metrics
func AddPrometheusMetric(gp *managedv1alpha1.GroupPermission) {
	addRBACClusterPermissionMetric(gp)
	addRBACNamespacePermissionMetric(gp)
}

// addRBACClusterPermissionMetric - add a GroupPermission to the exported data
// Iterates through the ClusterPermissions
func addRBACClusterPermissionMetric(gp *managedv1alpha1.GroupPermission) {
	for _, clusterPermissionName := range gp.Spec.ClusterPermissions {
		RBACClusterwidePermissions.With(prometheus.Labels{
			"group_name":              gp.Spec.GroupName,
...
// deleteRBACClusterPermissionMetric - delete a GroupPermission from the
// exported Prometheus data. Iterates through al the ClusterPermissions
func deleteRBACClusterPermissionMetric(gp *managedv1alpha1.GroupPermission) {
	var r bool
	for _, clusterPermissionName := range gp.Spec.ClusterPermissions {
		...
		if !r {
			log.Info(fmt.Sprintf("Failed to delete GaugeVec labels: group_name='%s', group_permission_name='%s', cluster_permission='%s', state='1'",
				gp.Spec.GroupName, gp.ObjectMeta.GetName(), clusterPermissionName))
		}
	}
}

// addRBACNamespacePermissionMetric - add a GroupPermission to the exported data
// Iterates through the ClusterPermissions
func addRBACNamespacePermissionMetric(gp *managedv1alpha1.GroupPermission) {
...
// deleteRBACNamespacePermissionMetric - delete a GroupPermission from the
// exported Prometheus data. Iterates through al the Permissions
func deleteRBACNamespacePermissionMetric(gp *managedv1alpha1.GroupPermission) {
	var r bool
	...
	for _, permission := range gp.Spec.Permissions {
		...
		if !r {
			log.Info(fmt.Sprintf("Failed to delete GaugeVec labels: group_name='%s', group_permission_name='%s', cluster_permission='%s', state='1'",
				gp.Spec.GroupName, gp.ObjectMeta.GetName(), permission.ClusterRoleName))
		}
	}
}
